...
import { cachedSearchMarket, cachedDiscoverGithub, githubCacheDir, npmCacheDir } from "./cache.js";
import { listLocalSkills, setSkillEnabled } from "./local.js";
import { fetchLocalizedReadme } from "./localize.js";
...
export function skillFusionRoutes(dshHome) {
  return [
    ...
    {
      // All skills DSH has locally (~/.dsh/skills + parked-disabled), for the 本地 tab.
      kind: "exact",
      path: `${PREFIX}/local`,
      handler: async (req, res) => {
        if (req.method !== "GET") return json(res, 405, { ok: false, error: "method-not-allowed" });
        json(res, 200, { ok: true, skills: listLocalSkills(dshHome) });
      },
    },
    {
      // Enable/disable a local skill (moves it between skills/ and disabled/).
      kind: "exact",
      path: `${PREFIX}/toggle`,
      handler: async (req, res) => {
        ...
        try {
          ...
          if (!name || typeof enabled !== "boolean") return json(res, 400, { ok: false, error: "name-and-enabled-required" });
          const r = setSkillEnabled(dshHome, name, enabled);
    ...
    {
      // Relevance-ranked search over the bundled Chinese index: matches name
      // (zh/en) + Chinese descriptions with intent/synonym expansion.
      kind: "exact",
      path: `${PREFIX}/search`,
      handler: async (req, res) => {
        if (req.method !== "GET") return json(res, 405, { ok: false, error: "method-not-allowed" });
        try {
          ...
          const index = JSON.parse(readFileSync(new URL("../data/zh-index.json", import.meta.url), "utf8"));
          const { rankIndex } = await import("./search.js");
          json(res, 200, { ok: true, hits: rankIndex(index, q, { limit }) });
        } catch (e) {
    ...
    {
      // Bundled Chinese skill index (curated mainstream skills with developer
      // info + Chinese descriptions). Offline — shipped inside the plugin.
      kind: "exact",
      path: `${PREFIX}/zh-index`,
      handler: async (req, res) => {
        if (req.method !== "GET") return json(res, 405, { ok: false, error: "method-not-allowed" });
        try {
          const indexPath = new URL("../data/zh-index.json", import.meta.url);
          const index = JSON.parse(readFileSync(indexPath, "utf8"));
          json(res, 200, { ok: true, index });
        } catch (e) {
          json(res, 200, { ok: false, error: "index-unavailable" });
        }
      },
    },
    {
      // Localized README for a market result: prefers Chinese variants
      // (README.zh-CN.md etc.), falls back to English. Cached locally.
      kind: "exact",
      path: `${PREFIX}/readme`,
      handler: async (req, res) => {
        if (req.method !== "GET") return json(res, 405, { ok: false, error: "method-not-allowed" });
        ...
        if (source === "github") {
          ...
          const path = url.searchParams.get("path") || ""; // skill dir, or "" for repo root
          if (!repo) return json(res, 400, { ok: false, error: "repo-required" });
          const r = await fetchLocalizedReadme(repo, ref, path, { dshHome });
          return json(res, 200, r ? { ok: true, ...r } : { ok: false, error: "not-found" });
        }
        if (source === "npm") {
          const name = url.searchParams.get("name");
          if (!name) return json(res, 400, { ok: false, error: "name-required" });
          let meta;
          try { meta = await (await fetch(`https://registry.npmjs.org/${encodeURIComponent(name)}/latest`)).json(); } catch { meta = null; }
          if (!meta || meta.error) return json(res, 200, { ok: false, error: "not-found" });
          return json(res, 200, { ok: true, text: (meta.readme || meta.description || "").slice(0, 4000), lang: "en", path: "npm" });
        }
        return json(res, 400, { ok: false, error: "unknown-source" });
      },
    },
    {
      kind: "exact",
      ...
      handler: async (req, res) => {
        ...
        if (source === "local") {
          ...
          const name = url.searchParams.get("name");
          if (name) {
            // Tarball-based: download once, walk for skills, share cache with activate.
            const r = await discoverNpm(name, { cacheDir: npmCacheDir(dshHome, name) });
            if (Array.isArray(r)) candidates = r;
            else if (r) candidates = [r];
          }
          ...
          const ref = url.searchParams.get("ref") || "main";
          if (repo) {
            // Cached repo inspection: first fetch hits network, repeats use cache.
            const r = await cachedDiscoverGithub(repo, { ref, dshHome, fresh: url.searchParams.get("fresh") === "1" });
            if (r) candidates = r.candidates;
          }
        } else if (source === "market") {
          // Empty/missing q → featured homepage; non-empty q → keyword search.
          // page param streams further result pages for infinite scroll.
          // Results cached locally (24h TTL); fresh=1 bypasses the cache.
          const page = parseInt(url.searchParams.get("page") || "1", 10) || 1;
          const fresh = url.searchParams.get("fresh") === "1";
          const r = await cachedSearchMarket(url.searchParams.get("q") || "", { page, dshHome, fresh });
        ...
        const payload = { ok: true, candidates };
        if (source === "market") payload.hasMore = candidates.length >= 10;
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (sourceKind === "local") {
            ...
            // Shared tarball cache with discovery — download once, walk from disk.
            const cacheDir = npmCacheDir(dshHome, pkgName);
            const cands = await discoverNpm(pkgName, { cacheDir });
            if (!Array.isArray(cands) || cands.length === 0) return json(res, 404, { ok: false, error: "not-found" });
            const cand = cands.find(c => c.name === name);
            if (!cand) return json(res, 404, { ok: false, error: "not-found" });
            const pkgDir = join(cacheDir, "package");
            const skillMdPath = join(pkgDir, cand.skillDir, "SKILL.md");
            if (!existsSync(skillMdPath)) return json(res, 404, { ok: false, error: "skill-not-in-tarball" });
            sourceDir = join(pkgDir, cand.skillDir);
            ...
            if (auditResult.verdict === "block") return json(res, 422, { ok: false, error: "blocked", flags: auditResult.flags });
            version = cand.version || sourceRef.split("@")[1];
          } else if (sourceKind === "github") {
            // sourceRef = "owner/repo@ref"
            const [ownerRepo, ref] = sourceRef.includes("@") ? sourceRef.split("@") : [sourceRef, "main"];
            // Shared tarball cache dir with inspection — download once, reuse here.
            const cacheDir = githubCacheDir(dshHome, ownerRepo, ref);
            const cands = await discoverGithub(ownerRepo, { ref, cacheDir });
            const cand = cands.find(c => c.name === name);
            if (!cand) return json(res, 404, { ok: false, error: "not-found" });
            // Resolve extracted root (download if inspection hasn't populated it yet)
