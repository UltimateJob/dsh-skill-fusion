...
export async function discoverNpm(name, { fetchFn = globalThis.fetch, cacheDir } = {}) {
  // Tarball-based discovery: the registry metadata has NO file list, so the
  // only reliable way to find skills inside is to download the package once.
  // When the extraction already exists, read local package.json — zero network.
  if (cacheDir) {
    const pkgDir = join(cacheDir, "package");
    if (existsSync(pkgDir)) {
      let meta = null;
      try { meta = JSON.parse(readFileSync(join(pkgDir, "package.json"), "utf8")); } catch {}
      const { skillsFromExtractedRoot } = await import("./github.js");
      return skillsFromExtractedRoot(pkgDir, {
        sourceKind: "npm",
        sourceRef: `${meta?.name || name}@${meta?.version || "0.0.0"}`,
  ...
  if (cacheDir && pkg.dist?.tarball) {
    const pkgDir = join(cacheDir, "package");
    const r = await fetchTarball(pkg.dist.tarball, cacheDir, { fetchFn });
    if (!r.ok) return [];
    const { skillsFromExtractedRoot } = await import("./github.js");
    return skillsFromExtractedRoot(pkgDir, {
      sourceKind: "npm",
  ...
  // Metadata fallback (no cacheDir): legacy single-candidate shape.
