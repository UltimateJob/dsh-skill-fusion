...
import { dedupeByLang } from "../lang.js";
...
/**
 * Discover skills from an extracted package tree on disk: walk for SKILL.md
 * files and parse frontmatter. skillDir is relative to pkgRoot (package root).
 */
export function skillsFromExtractedRoot(pkgRoot, { sourceKind = "github", sourceRef, version = null, commit = null } = {}) {
  const skillFiles = [];
  const walk = (dir) => {
    let entries;
  ...
  // Multi-language repos: keep one variant per skill (zh preferred, en fallback, ja dropped).
  return dedupeByLang(candidates);
}
...
export async function discoverGithub(ownerRepo, { ref = "main", fetchFn = globalThis.fetch, cacheDir, concurrency = 8, timeoutMs = 15000 } = {}) {
  ...
  if (cacheDir) {
    ...
    if (pkgRoot) return skillsFromExtractedRoot(pkgRoot, { sourceKind: "github", sourceRef: `${ownerRepo}@${ref}`, version: ref });
  ...
  // Multi-language repos: keep one variant per skill (zh preferred, en fallback, ja dropped).
  return dedupeByLang(results.filter(Boolean));
}
