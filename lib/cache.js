...
export async function cachedDiscoverGithub(ownerRepo, { ref = "main", dshHome, ttlMs = DEFAULT_TTL_MS, fresh = false, fetchFn = globalThis.fetch } = {}) {
  ...
  // Don't cache empty results — an empty inspection almost always means a
  // transient network failure (e.g. raw blocked), not "repo has no skills".
  if (dshHome && candidates.length > 0) writeCache(dshHome, key, stripInternal(candidates));
  return { candidates, cached: false };
}
