...
test("e2e: discover -> audit -> activate lands in ~/.dsh/skills and is discoverable", async () => {
  const home = freshHome();
  const src = fixtureSource(join(home, "src"));
  const out = [];
  const log = s => out.push(s);

  assert.equal(await runCli(["discover", "--local", src], { out: log, dshHome: home }), 0);
  assert.ok(out.join("\n").includes("adversarial-review"));

  out.length = 0;
  assert.equal(await runCli(["audit", "--local", src, "--name", "adversarial-review"], { out: log, dshHome: home }), 0);
  assert.ok(out.join("\n").startsWith("pass"));

  out.length = 0;
  assert.equal(await runCli(["activate", "--local", src, "--name", "adversarial-review"], { out: log, dshHome: home }), 0);
...
test("e2e: blocked skill (name conflict) does not activate", async () => {
  ...
  const out = [];
  assert.equal(await runCli(["activate", "--local", src, "--name", "adversarial-review"], { out: s => out.push(s), dshHome: home }), 1);
